...
describe("Real Estate Crowdfunding", () => {
  ...
  beforeEach(async () => {
    ...
    const contractFactory = await ethers.getContractFactory("RealEstateCrowdfunding", signers[0]);
    contract = (await contractFactory.deploy()) as RealEstateCrowdfunding;
    await contract.deployed();
  });

  // Test case for campaign creation
  it("should allow a user to create a campaign", async () => {
  ...
  it("should allow a user to contribute to a campaign", async () => {
    await contract.createCampaign(ethers.utils.parseEther("10"));
  ...
  it("should not allow a user to contribute more than the goal", async () => {
    await contract.createCampaign(ethers.utils.parseEther("10"));
  ...
  it("should allow the beneficiary to withdraw funds when the goal is met", async () => {
    await contract.createCampaign(ethers.utils.parseEther("10"));
    await contract.contribute(0, { value: ethers.utils.parseEther("10") });
  ...
  it("should not allow the beneficiary to withdraw funds before the goal is met", async () => {
    await contract.createCampaign(ethers.utils.parseEther("10"));
    await contract.contribute(0, { value: ethers.utils.parseEther("5") });
  ...
  it("should allow a user to refund their contribution if the goal is not met", async () => {
    await contract.createCampaign(ethers.utils.parseEther("10"));
    await contract.contribute(0, { value: ethers.utils.parseEther("5") });

    await ethers.provider.send("evm_increaseTime", [86400 * 7]); // simulate time passing
